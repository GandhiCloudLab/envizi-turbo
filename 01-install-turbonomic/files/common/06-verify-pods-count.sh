...
POD_COUNT=$(oc get pods -n $NAMESPACE | grep Running |  grep '1\/' | wc -l ) 
...
if [[ $POD_COUNT -gt $MIN_POD_COUNT ]]; then
  echo "Pods count in $NAMESPACE namespace is good enough ($POD_COUNT) and it is more than $MIN_POD_COUNT"; 
